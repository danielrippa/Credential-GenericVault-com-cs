    ...
    public class GenericVault
    {
        ...
        public int CollectCredential(string targetName, int persistence)
        {
            ...
            var uiInfo = new CREDUI_INFO
            {
                ...
                pszMessageText = $"Enter credentials for {targetName}"
            };
        ...
        public CredentialResult ReadCredential(string targetName)
        {
            if (string.IsNullOrEmpty(targetName))
            {
                return new CredentialResult { Success = false, Value = "Target name cannot be empty." };
            }
            ...
            if (CredRead(targetName, CRED_TYPE_GENERIC, 0, out IntPtr credPtr))
            {
                try
                {
                    ...
                    if (cred.CredentialBlobSize > 0 && cred.CredentialBlob != IntPtr.Zero)
                    {
                        ...
                        return new CredentialResult { Success = true, Value = password };
                    }
                    else
                    {
                        return new CredentialResult { Success = false, Value = "Credential found but contains no data." };
                    }
                }
                finally
                {
            ...
            else
            {
                ...
                string errorMessage = (lastError == ERROR_NOT_FOUND)
                    ? $"Credential for target '{targetName}' not found."
                    : $"CredRead failed with Win32 error code: {lastError}.";

                return new CredentialResult { Success = false, Value = errorMessage };
            }
        }
    }
}
